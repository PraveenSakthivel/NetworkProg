...
pthread_mutex_t closeThreadLock = PTHREAD_MUTEX_INITIALIZER;
/*
 * Function prototypes
 */
int parse_uri(char *uri, char *target_addr, char *path, int  *port);
void format_log_entry(char *logstring, struct sockaddr_in *sockaddr, char *uri, int size);
void proxy(void * connfd);
...
{
    int port;                 /* The port the proxy is listening on  y*/ 
    int clientlen;            /* Size in bytes of the client socket address y*/
    int connfd;                     /* socket desciptor for talkign wiht client y*/ 
    int listenfd;
    struct sockaddr_in clientaddr;  /* Clinet address structure */ 
    //Used to fix a bug
    int error = 0;                  /* Used to detect error in reading requests y */
    param * params;
    /* Check arguments */
    if (argc != 2) {
...
void proxy(void * params){
    param * parameters = (param *) params;
    int connfd = parameters->connfd;
    struct sockaddr_in clientaddr = parameters->clientaddr;
    ...
    pthread_mutex_lock(&closeThreadLock);  
    fprintf(log_file, "%s %d\n", log_entry, response_len);
    fflush(log_file);
    pthread_mutex_unlock(&closeThreadLock);
